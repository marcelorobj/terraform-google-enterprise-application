...
module "agent" {
  source = "../../modules/htc-infra/modules/builder"
  ...
  containers = {
    agent = {
      source = "${path.module}/modules/agent/src"
    },
    loadtest = {
      source = "${path.module}/modules/loadtest"
    },
    american_option = {
      source = "${path.module}/modules/american-option"
    }
  }
...
module "cloudrun" {
  source = "../../modules/htc-infra/agent/modules/run"
...
module "gke" {
  source = "../../modules/htc-infra/agent/modules/gke"
  ...
  gke_clusters = local.gke_cluster_names
...
module "bigquery_capture" {
  source     = "../../modules/htc-infra/modules/pubsub-subscriptions"
  project_id = var.infra_project
  # region                     = var.region
...
module "quota" {
  ...
  source = "../../modules/htc-infra/modules/quota"
